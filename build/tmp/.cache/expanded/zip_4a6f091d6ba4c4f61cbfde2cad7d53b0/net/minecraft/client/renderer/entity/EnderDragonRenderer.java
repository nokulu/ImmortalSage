...
public class EnderDragonRenderer extends EntityRenderer<EnderDragon> {
    ...
    public void render(EnderDragon pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        if (pEntity.dragonDeathTime > 0) {
            ...
            this.model.renderToBuffer(pPoseStack, vertexconsumer1, pPackedLight, OverlayTexture.pack(0.0F, flag));
        } else {
            VertexConsumer vertexconsumer2 = pBuffer.getBuffer(RENDER_TYPE);
            this.model.renderToBuffer(pPoseStack, vertexconsumer2, pPackedLight, OverlayTexture.pack(0.0F, flag));
        }

        VertexConsumer vertexconsumer3 = pBuffer.getBuffer(EYES);
        this.model.renderToBuffer(pPoseStack, vertexconsumer3, pPackedLight, OverlayTexture.NO_OVERLAY);
        if (pEntity.dragonDeathTime > 0) {
            float f3 = ((float)pEntity.dragonDeathTime + pPartialTicks) / 200.0F;
            pPoseStack.pushPose();
            pPoseStack.translate(0.0F, -1.0F, -2.0F);
            renderRays(pPoseStack, f3, pBuffer.getBuffer(RenderType.dragonRays()));
            renderRays(pPoseStack, f3, pBuffer.getBuffer(RenderType.dragonRaysDepth()));
            pPoseStack.popPose();
        }
        ...
        if (pEntity.nearestCrystal != null) {
            pPoseStack.pushPose();
            float f4 = (float)(pEntity.nearestCrystal.getX() - Mth.lerp((double)pPartialTicks, pEntity.xo, pEntity.getX()));
            float f5 = (float)(pEntity.nearestCrystal.getY() - Mth.lerp((double)pPartialTicks, pEntity.yo, pEntity.getY()));
            float f6 = (float)(pEntity.nearestCrystal.getZ() - Mth.lerp((double)pPartialTicks, pEntity.zo, pEntity.getZ()));
            renderCrystalBeams(f4, f5 + EndCrystalRenderer.getY(pEntity.nearestCrystal, pPartialTicks), f6, pPartialTicks, pEntity.tickCount, pPoseStack, pBuffer, pPackedLight);
            pPoseStack.popPose();
        }
    ...
    public static void renderCrystalBeams(
        float pX, float pY, float pZ, float pPartialTick, int pTickCount, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight
    ) {
        ...
        VertexConsumer vertexconsumer = pBufferSource.getBuffer(BEAM);
    ...
    public static class DragonModel extends EntityModel<EnderDragon> {
        ...
        public void renderToBuffer(PoseStack pPoseStack, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, int pColor) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                this.neck.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor);
            }
        ...
        private void renderSide(
            ...
            pRearLeg.xRot = 1.0F + pRotation * 0.1F;
            pRearLegTip.xRot = 0.5F + pRotation * 0.1F;
            pRearFoot.xRot = 0.75F + pRotation * 0.1F;
            pFrontLeg.xRot = 1.3F + pRotation * 0.1F;
            pFrontLegTip.xRot = -0.5F - pRotation * 0.1F;
            pFrontFoot.xRot = 0.75F + pRotation * 0.1F;
            pWing.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pAlpha);
            pFrontLeg.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pAlpha);
            pRearLeg.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pAlpha);
        }
    }
}