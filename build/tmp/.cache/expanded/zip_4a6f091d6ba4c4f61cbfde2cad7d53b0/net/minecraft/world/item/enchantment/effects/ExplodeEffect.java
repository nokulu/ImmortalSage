...
public record ExplodeEffect(
    ...
    public void apply(ServerLevel pLevel, int pEnchantmentLevel, EnchantedItemInUse pItem, Entity pEntity, Vec3 pOrigin) {
        Vec3 vec3 = pOrigin.add(this.offset);
        pLevel.explode(
            this.attributeToUser ? pEntity : null,
            this.getDamageSource(pEntity, vec3),
            new SimpleExplosionDamageCalculator(
                this.blockInteraction != Level.ExplosionInteraction.NONE,
                this.damageType.isPresent(),
                this.knockbackMultiplier.map(p_345148_ -> p_345148_.calculate(pEnchantmentLevel)),
                this.immuneBlocks
            ),
            vec3.x(),
            vec3.y(),
            vec3.z(),
            Math.max(this.radius.calculate(pEnchantmentLevel), 0.0F),
            this.createFire,
