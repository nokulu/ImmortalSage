...
public record ItemTrimPredicate(Optional<HolderSet<TrimMaterial>> material, Optional<HolderSet<TrimPattern>> pattern)
    ...
    public boolean matches(ItemStack pStack, ArmorTrim pValue) {
        return this.material.isPresent() && !this.material.get().contains(pValue.material())
            ? false
            : !this.pattern.isPresent() || this.pattern.get().contains(pValue.pattern());
    }
}