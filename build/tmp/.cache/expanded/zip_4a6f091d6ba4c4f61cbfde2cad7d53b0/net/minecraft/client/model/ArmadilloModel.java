...
public class ArmadilloModel extends AgeableHierarchicalModel<Armadillo> {
    ...
    public void setupAnim(Armadillo pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        if (pEntity.shouldHideInShell()) {
            ...
            this.head.xRot = Mth.clamp(pHeadPitch, -22.5F, 25.0F) * (float) (Math.PI / 180.0);
            this.head.yRot = Mth.clamp(pNetHeadYaw, -32.5F, 32.5F) * (float) (Math.PI / 180.0);
        }

        this.animateWalk(ArmadilloAnimation.ARMADILLO_WALK, pLimbSwing, pLimbSwingAmount, 16.5F, 2.5F);
