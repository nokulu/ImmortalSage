...
public abstract class PackDetector<T> {
    ...
    public T detectPackResources(Path pPath, List<ForbiddenSymlinkInfo> pForbiddenSymlinkInfos) throws IOException {
        return detectPackResources(pPath, pForbiddenSymlinkInfos, true);
    }
    @Nullable
    public T detectPackResources(Path pPath, List<ForbiddenSymlinkInfo> pForbiddenSymlinkInfos, boolean requireMeta) throws IOException {
        Path path = pPath;
