...
public class AdvancementWidget {
    ...
    public AdvancementWidget(AdvancementTab pTab, Minecraft pMinecraft, AdvancementNode pAdvancementNode, DisplayInfo pDisplay) {
        ...
        int j = 29 + pMinecraft.font.width(this.title) + i;
        this.description = Language.getInstance()
            .getVisualOrder(this.findOptimalLines(ComponentUtils.mergeStyles(pDisplay.getDescription().copy(), Style.EMPTY.withColor(pDisplay.getType().getChatColor())), j));

        for (FormattedCharSequence formattedcharsequence : this.description) {
            j = Math.max(j, pMinecraft.font.width(formattedcharsequence));
        }
