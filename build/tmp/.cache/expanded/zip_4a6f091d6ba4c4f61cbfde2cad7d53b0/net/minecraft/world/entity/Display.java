...
public abstract class Display extends Entity {
    ...
    public void onSyncedDataUpdated(EntityDataAccessor<?> pKey) {
        ...
        if (RENDER_STATE_IDS.contains(pKey.id())) {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
        pBuilder.define(DATA_POS_ROT_INTERPOLATION_DURATION_ID, 0);
        pBuilder.define(DATA_TRANSFORMATION_INTERPOLATION_START_DELTA_TICKS_ID, 0);
        pBuilder.define(DATA_TRANSFORMATION_INTERPOLATION_DURATION_ID, 0);
        pBuilder.define(DATA_TRANSLATION_ID, new Vector3f());
        pBuilder.define(DATA_SCALE_ID, new Vector3f(1.0F, 1.0F, 1.0F));
        pBuilder.define(DATA_RIGHT_ROTATION_ID, new Quaternionf());
        pBuilder.define(DATA_LEFT_ROTATION_ID, new Quaternionf());
        pBuilder.define(DATA_BILLBOARD_RENDER_CONSTRAINTS_ID, Display.BillboardConstraints.FIXED.getId());
        pBuilder.define(DATA_BRIGHTNESS_OVERRIDE_ID, -1);
        pBuilder.define(DATA_VIEW_RANGE_ID, 1.0F);
        pBuilder.define(DATA_SHADOW_RADIUS_ID, 0.0F);
        pBuilder.define(DATA_SHADOW_STRENGTH_ID, 1.0F);
        pBuilder.define(DATA_WIDTH_ID, 0.0F);
        pBuilder.define(DATA_HEIGHT_ID, 0.0F);
        pBuilder.define(DATA_GLOW_COLOR_OVERRIDE_ID, -1);
    }
    ...
    protected void addAdditionalSaveData(CompoundTag pCompound) {
        Transformation.EXTENDED_CODEC
            ...
            .ifSuccess(p_270528_ -> pCompound.put("transformation", p_270528_));
        Display.BillboardConstraints.CODEC
            .encodeStart(NbtOps.INSTANCE, this.getBillboardConstraints())
            .ifSuccess(p_270227_ -> pCompound.put("billboard", p_270227_));
        ...
        if (brightness != null) {
            Brightness.CODEC.encodeStart(NbtOps.INSTANCE, brightness).ifSuccess(p_270121_ -> pCompound.put("brightness", p_270121_));
        }
    }
    ...
    public static class BlockDisplay extends Display {
        ...
        protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
            ...
            pBuilder.define(DATA_BLOCK_STATE_ID, Blocks.AIR.defaultBlockState());
        }
    ...
    public static class ItemDisplay extends Display {
        ...
        protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
            super.defineSynchedData(pBuilder);
            pBuilder.define(DATA_ITEM_STACK_ID, ItemStack.EMPTY);
            pBuilder.define(DATA_ITEM_DISPLAY_ID, ItemDisplayContext.NONE.getId());
        }
        ...
        protected void readAdditionalSaveData(CompoundTag pCompound) {
            ...
            if (pCompound.contains("item")) {
                this.setItemStack(ItemStack.parse(this.registryAccess(), pCompound.getCompound("item")).orElse(ItemStack.EMPTY));
            } else {
        ...
        protected void addAdditionalSaveData(CompoundTag pCompound) {
            ...
            if (!this.getItemStack().isEmpty()) {
                pCompound.put("item", this.getItemStack().save(this.registryAccess()));
            }

            ItemDisplayContext.CODEC.encodeStart(NbtOps.INSTANCE, this.getItemTransform()).ifSuccess(p_270615_ -> pCompound.put("item_display", p_270615_));
        }
    ...
    public static class TextDisplay extends Display {
        ...
        protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
            super.defineSynchedData(pBuilder);
            pBuilder.define(DATA_TEXT_ID, Component.empty());
            pBuilder.define(DATA_LINE_WIDTH_ID, 200);
            pBuilder.define(DATA_BACKGROUND_COLOR_ID, 1073741824);
            pBuilder.define(DATA_TEXT_OPACITY_ID, (byte)-1);
            pBuilder.define(DATA_STYLE_FLAGS_ID, (byte)0);
        }

        @Override
        public void onSyncedDataUpdated(EntityDataAccessor<?> pKey) {
            super.onSyncedDataUpdated(pKey);
            if (TEXT_RENDER_STATE_IDS.contains(pKey.id())) {
                this.updateRenderState = true;
            }
        }
        ...
        protected void addAdditionalSaveData(CompoundTag pCompound) {
            super.addAdditionalSaveData(pCompound);
            pCompound.putString("text", Component.Serializer.toJson(this.getText(), this.registryAccess()));
            ...
            Display.TextDisplay.Align.CODEC
                ...
                .ifSuccess(p_271001_ -> pCompound.put("alignment", p_271001_));
        }
