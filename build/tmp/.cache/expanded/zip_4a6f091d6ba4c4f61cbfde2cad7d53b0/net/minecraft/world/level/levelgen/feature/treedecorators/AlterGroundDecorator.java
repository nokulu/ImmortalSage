    ...
    private void placeBlockAt(TreeDecorator.Context pContext, BlockPos pPos) {
        for (int i = 2; i >= -3; i--) {
            ...
            if (Feature.isGrassOrDirt(pContext.level(), blockpos)) {
                var state = this.provider.getState(pContext.random(), pPos);
                state = net.minecraftforge.event.ForgeEventFactory.alterGround(pContext.level(), pContext.random(), blockpos, state);
