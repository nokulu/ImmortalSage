...
public class RamTarget extends Behavior<Goat> {
    ...
    protected void tick(ServerLevel pLevel, Goat pOwner, long pGameTime) {
        ...
        if (!list.isEmpty()) {
            ...
            DamageSource damagesource = pLevel.damageSources().noAggroMobAttack(pOwner);
            if (livingentity.hurt(damagesource, (float)pOwner.getAttributeValue(Attributes.ATTACK_DAMAGE))) {
                EnchantmentHelper.doPostAttackEffects(pLevel, livingentity, damagesource);
            }
