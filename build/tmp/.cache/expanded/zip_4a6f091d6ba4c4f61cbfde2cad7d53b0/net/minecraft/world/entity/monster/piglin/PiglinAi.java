...
public class PiglinAi {
    ...
    public static void angerNearbyPiglins(Player pPlayer, boolean pAngerOnlyIfCanSee) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !pAngerOnlyIfCanSee || BehaviorUtils.canSee(p_34881_, pPlayer)).forEach(p_341457_ -> {
            if (p_341457_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_341457_, pPlayer);
            } else {
                setAngerTarget(p_341457_, pPlayer);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
        if (!(pTarget instanceof Piglin)) {
            ...
            getAvoidTarget(pPiglin).ifPresent(p_341460_ -> {
                if (p_341460_.getType() != pTarget.getType()) {
    ...
    protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
        getAdultPiglins(pPiglin).forEach(p_341453_ -> {
            if (pTarget.getType() != EntityType.HOGLIN || p_341453_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_341453_, pTarget);
            }
        });
    }
