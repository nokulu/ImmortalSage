...
public class SupportBlockRenderer implements DebugRenderer.SimpleDebugRenderer {
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBufferSource, double pCamX, double pCamY, double pCamZ) {
        ...
        if (player != null && player.mainSupportingBlockPos.isPresent()) {
            this.drawHighlights(pPoseStack, pBufferSource, pCamX, pCamY, pCamZ, player, () -> 0.0, 1.0F, 0.0F, 0.0F);
        }

        for (Entity entity1 : this.surroundEntities) {
            if (entity1 != player) {
                this.drawHighlights(pPoseStack, pBufferSource, pCamX, pCamY, pCamZ, entity1, () -> this.getBias(entity1), 0.0F, 1.0F, 0.0F);
            }
        }
    }
