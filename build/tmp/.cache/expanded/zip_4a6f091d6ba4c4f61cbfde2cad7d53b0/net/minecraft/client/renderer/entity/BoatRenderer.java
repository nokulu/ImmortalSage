...
public class BoatRenderer extends EntityRenderer<Boat> {
        ...
        this.boatResources = Stream.of(Boat.Type.values())
            .collect(
                ImmutableMap.toImmutableMap(
                    p_173938_ -> (Boat.Type)p_173938_, p_340938_ -> Pair.of(getTextureLocation(p_340938_, pChestBoat), this.createBoatModel(pContext, p_340938_, pChestBoat))
                )
            );
    }
    ...
    private static ResourceLocation getTextureLocation(Boat.Type pType, boolean pChestBoat) {
        return pChestBoat
            ? ResourceLocation.withDefaultNamespace("textures/entity/chest_boat/" + pType.getName() + ".png")
            : ResourceLocation.withDefaultNamespace("textures/entity/boat/" + pType.getName() + ".png");
    }
    ...
    public void render(Boat pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        listmodel.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY);
