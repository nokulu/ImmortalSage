...
public record Tool(List<Tool.Rule> rules, float defaultMiningSpeed, int damagePerBlock) {
    ...
    public static record Rule(HolderSet<Block> blocks, Optional<Float> speed, Optional<Boolean> correctForDrops) {
        ...
        public static Tool.Rule minesAndDrops(List<Block> pBlocks, float pSpeed) {
            return forBlocks(pBlocks, Optional.of(pSpeed), Optional.of(true));
        }

        public static Tool.Rule minesAndDrops(TagKey<Block> pBlocks, float pSpeed) {
            return forTag(pBlocks, Optional.of(pSpeed), Optional.of(true));
        }

        public static Tool.Rule deniesDrops(TagKey<Block> pBlocks) {
            return forTag(pBlocks, Optional.empty(), Optional.of(false));
        }

        public static Tool.Rule overrideSpeed(TagKey<Block> pBlocks, float pSpeed) {
            return forTag(pBlocks, Optional.of(pSpeed), Optional.empty());
        }

        public static Tool.Rule overrideSpeed(List<Block> pBlocks, float pSpeed) {
            return forBlocks(pBlocks, Optional.of(pSpeed), Optional.empty());
        }

        private static Tool.Rule forTag(TagKey<Block> pTag, Optional<Float> pSpeed, Optional<Boolean> pCorrectForDrops) {
            return new Tool.Rule(BuiltInRegistries.BLOCK.getOrCreateTag(pTag), pSpeed, pCorrectForDrops);
        }

        private static Tool.Rule forBlocks(List<Block> pBlocks, Optional<Float> pSpeed, Optional<Boolean> pCorrectForDrops) {
            return new Tool.Rule(HolderSet.direct(pBlocks.stream().map(Block::builtInRegistryHolder).collect(Collectors.toList())), pSpeed, pCorrectForDrops);
        }
    }
}