...
public class Vindicator extends AbstractIllager {
    ...
    public void applyRaidBuffs(ServerLevel pLevel, int pWave, boolean pUnused) {
        ...
        if (flag) {
            ResourceKey<EnchantmentProvider> resourcekey = pWave > raid.getNumGroups(Difficulty.NORMAL)
                ? VanillaEnchantmentProviders.RAID_VINDICATOR_POST_WAVE_5
                : VanillaEnchantmentProviders.RAID_VINDICATOR;
            EnchantmentHelper.enchantItemFromProvider(itemstack, pLevel.registryAccess(), resourcekey, pLevel.getCurrentDifficultyAt(this.blockPosition()), this.random);
        }
