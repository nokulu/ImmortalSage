...
public record VanillaEquipmentLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
    ...
    public static LootTable.Builder trialChamberEquipment(Item pHelmet, Item pChestplate, ArmorTrim pTrim, HolderLookup.RegistryLookup<Enchantment> pEnchantments) {
        return LootTable.lootTable()
            .withPool(
                LootPool.lootPool()
                    ...
                    .add(
                        LootItem.lootTableItem(pHelmet)
                            .apply(SetComponentsFunction.setComponent(DataComponents.TRIM, pTrim))
                            .apply(
                                new SetEnchantmentsFunction.Builder()
                                    .withEnchantment(pEnchantments.getOrThrow(Enchantments.PROTECTION), ConstantValue.exactly(4.0F))
                                    .withEnchantment(pEnchantments.getOrThrow(Enchantments.PROJECTILE_PROTECTION), ConstantValue.exactly(4.0F))
                                    .withEnchantment(pEnchantments.getOrThrow(Enchantments.FIRE_PROTECTION), ConstantValue.exactly(4.0F))
                            )
                    )
            )
            .withPool(
                LootPool.lootPool()
                    ...
                    .when(LootItemRandomChanceCondition.randomChance(0.5F))
                    .add(
                        LootItem.lootTableItem(pChestplate)
                            .apply(SetComponentsFunction.setComponent(DataComponents.TRIM, pTrim))
                            .apply(
                                new SetEnchantmentsFunction.Builder()
                                    .withEnchantment(pEnchantments.getOrThrow(Enchantments.PROTECTION), ConstantValue.exactly(4.0F))
