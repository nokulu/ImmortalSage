...
public class WeightedBakedModel implements net.minecraftforge.client.model.IDynamicBakedModel {
    ...
    public WeightedBakedModel(List<WeightedEntry.Wrapper<BakedModel>> pList) {
        ...
        this.wrapped = pList.get(0).data();
    }

    // FORGE: Implement our overloads (here and below) so child models can have custom logic
    @Override
    public List<BakedQuad> getQuads(@Nullable BlockState pState, @Nullable Direction pDirection, RandomSource pRandom, net.minecraftforge.client.model.data.ModelData modelData, @Nullable net.minecraft.client.renderer.RenderType renderType) {
        return WeightedRandom.getWeightedItem(this.list, Math.abs((int)pRandom.nextLong()) % this.totalWeight)
            .map(p_325578_ -> p_325578_.data().getQuads(pState, pDirection, pRandom, modelData, renderType))
