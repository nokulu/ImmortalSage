...
public class Warden extends Monster implements VibrationSystem {
    ...
    public Packet<ClientGamePacketListener> getAddEntityPacket(ServerEntity pEntity) {
        return new ClientboundAddEntityPacket(this, pEntity, this.hasPose(Pose.EMERGING) ? 1 : 0);
    }
    ...
    public void readAdditionalSaveData(CompoundTag pCompound) {
        ...
        if (pCompound.contains("anger")) {
            AngerManagement.codec(this::canTargetEntity)
                .parse(registryops, pCompound.get("anger"))
        ...
        if (pCompound.contains("listener", 10)) {
            VibrationSystem.Data.CODEC
                .parse(registryops, pCompound.getCompound("listener"))
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        ...
        if (!this.level().isClientSide && !this.isNoAi() && !this.isDiggingOrEmerging()) {
            ...
            if (this.brain.getMemory(MemoryModuleType.ATTACK_TARGET).isEmpty()
                ...
                && (pSource.isDirect() || this.closerThan(livingentity, 5.0))) {
